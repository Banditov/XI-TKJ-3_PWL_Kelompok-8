    ...
    <div id="postsContainer" class="flex flex-col gap-10">
        <?php if (empty($posts)): ?>
            <div class="w-full rounded-4xl bg-white text-[#545F71] drop-shadow-lg p-10 text-center">
                ...
                <a href="/posts/create"
                    class="inline-block mt-4 px-6 py-3 bg-[#2C7CFF] text-white rounded-full hover:bg-white hover:text-[#2C7CFF] hover:ring-2 transition">Create
                    your first post!</a>
            </div>
        <?php else: ?>
            <?php foreach ($posts as $index => $post): ?>
                <div
                    class="w-full rounded-4xl bg-white text-[#545F71] drop-shadow-lg post hover:drop-shadow-[0_0_10px_rgba(0,0,0,0.5)] transition">
