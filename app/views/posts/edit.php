...
<?php endif; ?>
        <form action="/posts/<?= $post['id'] ?>/update" method="POST" id="postForm" class="flex flex-col gap-5">
            ...
            <div class="flex gap-4 w-full flex-col">
                ...
                <button type="button" onclick="showDeletePostModal(<?= $post['id'] ?>)" class="px-6 py-3 bg-red-600 text-white rounded-full w-full hover:bg-white hover:text-red-600 hover:ring-2 red transition cursor-pointer">
                    Delete Post
                </button>
                <form id="deletePostForm-<?= $post['id'] ?>" action="/posts/<?= $post['id'] ?>/delete" method="POST" class="hidden"></form>
