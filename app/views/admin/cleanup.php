    ...
    <div class="w-full rounded-4xl bg-white text-[#545F71] drop-shadow-lg p-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="bg-blue-50 p-6 rounded-xl text-center">
                <p class="text-3xl font-bold text-blue-600"><?= $totalImages ?></p>
                <p class="text-gray-600">Total Images</p>
            </div>
            <div class="bg-green-50 p-6 rounded-xl text-center">
                <p class="text-3xl font-bold text-green-600"><?= $usedImages ?></p>
                <p class="text-gray-600">Used Images</p>
            </div>
            <div class="bg-red-50 p-6 rounded-xl text-center">
                <p class="text-3xl font-bold text-red-600"><?= $unusedCount ?></p>
        ...
        <?php if ($unusedCount > 0): ?>
            ...
            <form action="/admin/cleanup/run" method="POST" onsubmit="return confirm('Are you sure you want to delete <?= $unusedCount ?> unused image(s)? This action cannot be undone.')">
                <button type="submit" class="px-6 py-3 bg-red-600 text-white rounded-full hover:bg-white hover:text-red-600 hover:ring-2 transition cursor-pointer w-full">
        ...
        <div class="mt-6">
            <a href="/posts" class="text-[#2C7CFF] hover:underline">← Back to Posts</a>
        </div>
    </div>
</main>